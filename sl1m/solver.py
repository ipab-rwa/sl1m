...
def solve_lp_linprog(q, G=None, h=None, C=None, d=None):
    ...
    """
    t_init = clock()
    bounds = [(-100000., 10000.) for _ in range(q.shape[0])]
    res = linprog(q, A_ub=G, b_ub=h, A_eq=C, b_eq=d, bounds=bounds,
                  method='interior-point', callback=None, options={'presolve': True})
    t_end = clock()
...
def solve_lp_glpk(q, G=None, h=None, C=None, d=None):
    ...
    lp.simplex()
    t_end = clock()
    return ResultData(lp.status == "opt", ms(t_end-t_init), np.array([c.primal for c in lp.cols]))
...
def solve_lp_gurobi(q, G=None, h=None, C=None, d=None):
    ...
    cVars = []
    for el in (q):
        cVars.append(model.addVar(vtype=grb.GRB.CONTINUOUS,
...
def solve_MIP_gurobi(slack_selection_vector, G=None, h=None, C=None, d=None):
    ...
    for variable in (slack_selection_vector):
        if variable > 0:
            ...
            cVars.append(model.addVar(lb=-grb.GRB.INFINITY, ub=grb.GRB.INFINITY,
                                      vtype=grb.GRB.CONTINUOUS, name='x'))
